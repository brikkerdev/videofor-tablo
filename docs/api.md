...
Если задан `API_KEY`, токен `X-API-Key` обязателен для всех `/api/*`, кроме `POST /api/events` (свой контроль того же ключа) и открытых `GET /health`, `/`, `/ui/`. Если `API_KEY` пуст — управляющие эндпоинты открыты (режим доверенной сети). Веб-интерфейс хранит ключ в браузере (`localStorage`, поле «API-ключ» на вкладке «Подключение») и подставляет его в каждый запрос; после включения `API_KEY` введите ключ один раз.

Кросс-доменный доступ ограничен `CORS_ORIGINS` (по умолчанию пусто — только same-origin, как сам UI). `GET /api/tablo/connection` отдаёт `token` и `matrix_pass`: держите его за `API_KEY` или закрытой сетью. Если вынесете API наружу — добавьте сетевой барьер или обратный прокси.
...
| 401 | Неверный/отсутствующий `X-API-Key` (когда задан `API_KEY`): на `/api/events` или на любом управляющем `/api/*` (`Unauthorized`) |
...
`device` — реальные размеры панели, прочитанные у шлюза через `GET /connect.json` (или дефолт 256×96).
...
**LineConfig:** `key` (ключ показателя), `enabled`, `color`, `brightness`, `threshold`.

**Threshold:** `value`, `op` (`>=`/`>`/`<=`/`<`/`==`), `color`, `target` (`line` — красить всю строку, иначе только значение), `duration_seconds`. При `duration_seconds: 0` подсветка держится, пока значение удовлетворяет условию. При `duration_seconds > N` подсветка включается на N секунд при каждом срабатывании или изменении значения, затем гаснет автоматически.
...
`reachable` — задан адрес шлюза; `connected` — шлюз подтвердил связь с матрицей (источник правды `GET /connect.json`, см. `refresh_status` в `app/tablo.py`); `last_push` — результат последней попытки доставки (`null`, если ещё не было).
...
### GET /api/tablo/connection

Текущие параметры подключения к шлюзу. Значения берутся из таблицы `config`, при отсутствии — из переменных окружения.

```json
{
  "url": "http://192.168.0.64",
  "token": "",
  "matrix_ip": "192.168.0.47",
  "matrix_pass": "guest",
  "active_url": "http://192.168.0.64"
}
```

`active_url` — адрес, на котором клиент работает сейчас (после последнего `reconfigure`).

> Ответ содержит `token` и `matrix_pass` открытым текстом. Эндпоинт без аутентификации — держать только в доверенной сети (см. раздел «Аутентификация»).

### PUT /api/tablo/connection

Сменить параметры подключения в рантайме без рестарта. Сохраняет значения в `config` и переподключает клиент (`reconfigure`) в фоне.

**Тело:** `{ "url": "http://192.168.0.64", "token": "", "matrix_ip": "192.168.0.47", "matrix_pass": "guest" }`

**Ответ:** `{ "status": "ok" }`

### GET /api/tablo/push-log

Журнал последних попыток доставки на табло (кольцевой буфер в памяти).

```json
[
  { "at": "10:15:05", "ok": true, "error": "", "trigger": "event", "details": "Нарушения за день +1" }
]
```

`trigger` — что вызвало push (`event`, `config`, `adjust`, `reload`, `push`, `clock`, `startup`); `details` — расшифровка (для `event` — какие показатели и на сколько изменились).
